...
export function QuestionnaireFormModal({
  ...
  const [questions, setQuestions] = useState<QuestionDraft[]>([emptyDraft()]);
  ...
  const buildQuestions = (): { questions?: Question[]; error?: string } => {
    if (questions.length === 0) {
      return { error: 'Ajoutez au moins une question au questionnaire.' };
    }
        ...
        <Card shadow="xs" p="md" radius="md" withBorder>
          ...
          <Stack gap="md">
            <TextInput
              ...
              placeholder="Ex: Évaluation à chaud - Formations techniques"
              required
              value={nom}
